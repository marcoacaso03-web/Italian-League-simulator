...
export default function LobbyRoomPage({ lobbyCode, onStartGame }: LobbyRoomPageProps) {
  ...
  }, [lobbyCode, onStartGame, playerId, currentPlayer]);
      ...
      <div className="w-full max-w-lg space-y-5">
        ...
        {lobbyConfig && (
          <div className="glass rounded-2xl p-4">
            ...
            <div className="flex flex-wrap gap-2 text-xs">
              ...
              <span className="bg-white/5 rounded-lg px-2 py-1 text-slate-300">
                📐 {lobbyConfig.formation}
              </span>
              <span className="bg-white/5 rounded-lg px-2 py-1 text-slate-300">
                🎰 {lobbyConfig.draftMode === 'squad_first' ? 'Squadra' : 'Ruolo'} Prima
              </span>
              <span className="bg-white/5 rounded-lg px-2 py-1 text-slate-300">
                📅 {lobbyConfig.eraFrom}–{lobbyConfig.eraTo}
              </span>
            </div>
          </div>
        )}
          ...
          {players.map((player) => {
            ...
            return (
              ...
              >
                ...
                {/* Host badge */}
                {player.is_host && (
                  <span className="text-[10px] font-bold text-amber-400 bg-amber-500/10 rounded px-1.5 py-0.5">
                    👑 HOST
                  </span>
                )}
                ...
                <span className={`text-xs font-semibold ${
                  player.is_ready ? 'text-emerald-400' : 'text-slate-500'
                }`}>
        ...
        <div className="space-y-3">
          ...
          {isHost && (
            ...
            >
              {starting ? '⏳ Avvio...' : allReady ? '🚀 Avvia Partita!' : `⏳ Aspettando giocatori... (${players.filter(p => p.is_ready).length}/${players.length - 1})`}
            </button>
          )}
