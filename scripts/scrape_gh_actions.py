#!/usr/bin/env python3
"""
Scraping FBref per Serie A — GitHub Actions (Playwright async)

Uses Playwright (pre-installed on GH Actions) with Chromium.
Playwright bypasses Cloudflare because it's a real browser with JS execution.
...
async def fetch_page(url):
    """Fetch page with Playwright — real browser, bypasses Cloudflare."""
...
def extract_all_tables(html):
    ...
    for comment in soup.find_all(string=lambda text: isinstance(text, Comment)):
        ...
        try:
            for t in BeautifulSoup(ct, "html.parser").find_all("table"):
                tid = t.get("id", "")
...
async def get_all_seasons():
    ...
    for a in soup.find_all("a", href=True):
        ...
        if m: seasons[m.group(1)] = m.group(1); continue
...
async def scrape_season(label, slug):
    ...
    if d:
        ...
        for row in parse_rows(tables[d]):
            ...
            if key in players:
                ...
                players[key]["errors"] = _si(row.get("errors","0")); m += 1
...
async def main():
    ...
    for i, (label, slug) in enumerate(pbar):
        ...
        try:
            clubs, players = await scrape_season(label, slug)
        except Exception as e:
            log.error(f"  ❌ {e}")
